...
class ServerRequest extends Request implements ServerRequestInterface
{
    ...
    public static function getUriFromGlobals()
    {
        ...
        // 视图获取访问 端口号PORT 与 主机HOST
        $hasPort = false;
        if (isset($_SERVER['HTTP_HOST'])) {                 // http请求头中的 Host
        ...
        } elseif (isset($_SERVER['SERVER_NAME'])) {         // 服务器主机名(可能伪造)
            $uri = $uri->withHost($_SERVER['SERVER_NAME']);
        } elseif (isset($_SERVER['SERVER_ADDR'])) {         // 服务器ip地址
...
//        $headers = \getallheaders();
    ...
    private function filterUploadedFiles($uploadedFiles)
    {
        if ($uploadedFiles instanceof UploadedFileInterface) {
            return true;
        } elseif (is_array($uploadedFiles)) {
            foreach ($uploadedFiles as $k => $v) {
                $valid = $this->filterUploadedFiles($v);
                if ($valid !== true) {
                    throw new \InvalidArgumentException("uploadedFiles structure is invalid");
                }
            }
        }
